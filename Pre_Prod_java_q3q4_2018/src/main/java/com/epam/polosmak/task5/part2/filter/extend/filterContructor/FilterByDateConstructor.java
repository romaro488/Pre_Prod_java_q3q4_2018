package com.epam.polosmak.task5.part2.filter.extend.filterContructor;

import com.epam.polosmak.task5.part2.constants.constants;
import com.epam.polosmak.task5.part2.filter.SourceConstructor;
import com.epam.polosmak.task5.part2.filter.extend.filter.Filter;
import com.epam.polosmak.task5.part2.filter.extend.filter.FilterByDate;

import java.time.LocalDate;

public class FilterByDateConstructor extends SourceConstructor {

    @Override
    public Filter construct(Filter filter) {
        printMessage.print(constants.FIND_FILE_BY_DATE);
        if (inputHandler.getSelect() != AGREE) {
            return filter;
        }
        printMessage.println(constants.ENTER_PERIOD_DATE);
        LocalDate start = inputHandler.getInputDateTime();
        LocalDate end = inputHandler.getInputDateTime();
        return new FilterByDate(filter, start, end);
    }
}